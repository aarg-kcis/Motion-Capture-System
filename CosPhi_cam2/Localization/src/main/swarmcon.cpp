...
float fieldLength = 2.88;
float fieldWidth = 2.05;
//----------------------------------------------------------------------------
...
float cameraHeight = 2.6;		//height of the camera above the field 
//----------------------------------------------------------------------------
	...
	while (stop == false)
	{
		...
		for (int i = 0;i<numBots;i++){
			if (currentSegmentArray[i].valid){
				...
				numFound++;
				//printf("Valid ID : %i \n", objectArray[i].ID); //Pulkit
				validIdArray[i] =  objectArray[i].ID;
				if (currentSegmentArray[i].x == lastSegmentArray[i].x) numStatic++;
			}
		}
		//printf("Valid ID %i  %i %i\n", validIdArray[0],validIdArray[1], validIdArray[2]); //Pulkit
		...
		server->clearToSend(validIdArray); //Pulkit
		
		std::fill(validIdArray, validIdArray+10, -1); //Pulkit
